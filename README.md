...
04:45にLuna（推論 `max`）で2系統のイベント調査を同時に開始し、06:25に検証済みデータだけをローカル公開、06:35にGitHub Pagesへ送信します。GitHub Pagesのデプロイ時間を25分見込み、通常は07:00までにオンライン版の更新が完了する構成です。各系統16〜24回、合計32〜48回を目安に異なる検索・サイト確認を行うため、通常調査のおよそ2倍の探索量です。各調査には最大75分を確保し、どちらかの調査や検証に失敗した場合は、現在公開中のデータを壊さず保持します。
