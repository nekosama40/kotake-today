...
$generateTrigger = New-ScheduledTaskTrigger -Daily -At '05:30'
$publishTrigger = New-ScheduledTaskTrigger -Daily -At '07:00'
$onlineTrigger = New-ScheduledTaskTrigger -Daily -At '07:05'
...
Register-ScheduledTask -TaskName 'KotakeEvents-Generate' -Action $generateAction -Trigger $generateTrigger -Principal $principal -Settings $generateSettings -Description 'Research Tokyo events with two parallel Luna max passes at 05:30' -Force | Out-Null
Register-ScheduledTask -TaskName 'KotakeEvents-Publish' -Action $publishAction -Trigger $publishTrigger -Principal $principal -Settings $publishSettings -Description 'Publish validated event data at 07:00' -Force | Out-Null
Register-ScheduledTask -TaskName 'KotakeEvents-Online' -Action $onlineAction -Trigger $onlineTrigger -Principal $principal -Settings $onlineSettings -Description 'Push validated event data to GitHub Pages at 07:05' -Force | Out-Null
